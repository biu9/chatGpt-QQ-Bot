...
async function readConversation(uid) {
  return new Promise((resolve,reject) => {
    if(fs.existsSync('./conversation/'+uid+'.txt')) {
      resolve(fs.readFileSync('./conversation/'+uid+'.txt', 'utf8'))
    } else {
      fs.writeFileSync('./conversation/'+uid+'.txt',preConversation)
      resolve(preConversation)
    }
  })
}
...
export async function getGptReply(input,uid) {
  ...
  const conversation = await readConversation(uid);
